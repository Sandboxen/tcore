--FROM https://github.com/PAC3-Server/notagain/blob/master/lua/notagain/essential/libraries/easylua.lua
local easylua = {}
...
function easylua.Start(ply)
	if started then
		Msg"[ELua] "print("Session not ended for ",_G.me or (easylua.vars and easylua.vars.me),", restarting session for",ply)
		easylua.End()
	end
	started = true
...
function easylua.End()
	...
	if easylua.vars then
		for key, value in pairs(easylua.vars) do
			if easylua.oldvars and easylua.oldvars[key] then
				_G[key] = easylua.oldvars[key]
			else
				_G[key] = nil
			end
		end
...
function easylua.RunLua(ply, code, env_name)
	...
	easylua.Start(ply)
		if easylua.vars then
			...
			for key, value in next,(easylua.vars or {}) do
