				...
				if is_dancing(LocalPlayer()) then
					if cmd:KeyDown(IN_JUMP) then
						if not suppress then
                            ...
                            --chat.AddText(Color(128,0,255),"[SERVER] ",Color(255,255,255),"BPM: ",bpm:GetInt())
							suppress = true
						end
					else
						suppress = false
...
if SERVER then
	...
	concommand.Add(tag .. "_setrate", function(ply, _, args)
        ...
        ply:PrintMessage( HUD_PRINTCENTER , "BPM:".. tonumber(args[1]))
	end)
    concommand.Add("dance",function(ply)
        set_dancing(ply,not is_dancing(ply))
    end)
end